...
SCENARIOS: tuple[Scenario, ...] = (
    ...
    Scenario(
        name="duplicate_charges",
        question=(
            "Existem cobranças duplicadas em julho de 2026?"
        ),
        expected_tools=(
...
def main() -> int:
    ...
    print(
        "Data source: real local PostgreSQL"
    )
    print(
        "Model: approved local Ollama runtime"
    )
    ...
    if all(results):
        print(
            "RESULT: ALL SELECTED CAPABILITIES PASSED"
        )
        return 0

    print(
        "RESULT: ONE OR MORE CAPABILITIES FAILED"
    )
    return 1
